        ...
        if ($row['status'] !== 'cancelled') {
            ...
            try {
                ...
                $insert_query = "INSERT INTO cancelled_bookings (appointment_id, full_name, service_names, 
                                appointment_date, appointment_time, payment_method, total_price, reason, cancelled_at) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
                $insert_stmt = $conn->prepare($insert_query);
                $insert_stmt->bind_param('issssssd', $row['id'], $row['full_name'], $row['service_names'], 
                                          $row['appointment_date'], $row['appointment_time'], 
                                          $row['payment_method'], $row['total_price'], $reason);
                $insert_stmt->execute();
